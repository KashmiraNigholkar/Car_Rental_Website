...
const Slidebar = () => {
  ...
  return (
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full borser-r border-borderColor text-sm'>
        ...
        <div className='w-full'>
            {ownerMenuLinks.map((link,index)=>(
                <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path===location.pathname ?'bg-primary/10 text-primary':'text-gray-600'}`}>
                    ...
                    <div className={`${link.path==location.pathname && 'bg-primary'} w-1.5 h-8 roundedl right-0 absolute`}>
